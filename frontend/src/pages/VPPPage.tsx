...
function KPI({ label, value, sub, accent = 'text-[#E8ECF1]' }: {
...
function SocBar({ soc }: { soc: number }) {
  const color = soc < 30 ? 'bg-[#E5484D]' : soc < 60 ? 'bg-[#E8A33D]' : 'bg-[#2EBD85]';
  return (
    <div className="w-full bg-[#222933] rounded-full h-2">
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-2xl font-bold text-[#E8ECF1]">VPP 통합 관제</h1>
        ...
        <div className="flex items-center gap-3">
          <Link
            to="/control-room"
            className="rounded-md bg-[#4C8DFF] px-4 py-2 text-sm font-semibold text-white hover:bg-[#3D75E0]"
          >
            🖥 관제센터 모드
          </Link>
          <span className={`h-2.5 w-2.5 rounded-full ${venState === 'responding' ? 'bg-[#E8A33D] animate-pulse' : 'bg-[#2EBD85]'}`} />
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <KPI label="가용 유연성" value={`${p.available_flexibility_kw.toLocaleString()} kW`} sub="전력거래소 입찰 가능량" accent="text-[#4C8DFF]" />
        <KPI
          label="DR 1회 순수익"
      ...
      <Card title="단지 포트폴리오" subtitle={p.note ?? ''}>
        <div className="grid gap-4 md:grid-cols-3">
          {p.buildings.map((b) => (
            <div key={b.building_id} className="rounded-md border border-[#222933] bg-[#0A0C10] p-5">
              <div className="flex items-center justify-between mb-3">
                <p className="font-semibold text-[#E8ECF1]">{b.building_id}</p>
                <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${
                  b.live ? 'bg-[#2EBD85]/15 text-[#2EBD85]' : 'bg-[#4C8DFF]/15 text-[#4C8DFF]'
                }`}>
                  {b.live ? 'LIVE 실측' : '시뮬레이션'}
                </span>
              </div>
              <div className="flex justify-between text-sm mb-1">
                <span className="text-[#98A2B3]">SOC</span>
                <span className="font-semibold text-[#E8ECF1] tabular-nums">{b.current_soc}%</span>
              </div>
              <SocBar soc={b.current_soc} />
              <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
                <div>
                  <p className="text-xs text-[#98A2B3]">용량</p>
                  <p className="font-semibold text-[#E8ECF1] tabular-nums">{b.ess_capacity} kWh</p>
                </div>
                <div>
                  <p className="text-xs text-[#98A2B3]">가용 출력</p>
                  <p className="font-semibold text-[#4C8DFF] tabular-nums">{b.available_kw} kW</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card title="VTN 수요반응 발령" subtitle="OpenADR 2.0b 신호 체계">
          ...
          <div className="mt-4 border-t border-[#222933] pt-4">
            <p className="text-xs font-medium uppercase tracking-wider text-[#98A2B3] mb-3">발표 시연 시나리오</p>
            <div className="grid grid-cols-2 gap-3">
          ...
          {lastResult && (
            <div className="mt-4 rounded-md border border-[#222933] bg-[#0A0C10] px-4 py-3 text-sm text-[#E8ECF1]">
              {lastResult}
            </div>
          )}
          {activeEvent && (
            <div className="mt-3 rounded-md border border-[#E8A33D]/40 bg-[#E8A33D]/10 px-4 py-3 text-sm text-[#E8A33D]">
        ...
        <Card title="수익 현황" subtitle="수요반응 · 차익거래">
          <div className="divide-y divide-[#222933]">
            <div className="pb-3">
              <p className="text-xs font-medium uppercase tracking-wider text-[#98A2B3] mb-2">수요반응 (DR)</p>
              <div className="space-y-1.5 text-sm">
                <div className="flex justify-between"><span className="text-[#98A2B3]">감축량</span><span className="text-[#E8ECF1] font-semibold">{drRev ? `${drRev.reduction_kwh} kWh` : '—'}</span></div>
                <div className="flex justify-between"><span className="text-[#98A2B3]">총수익</span><span className="text-[#E8ECF1] font-semibold">{drRev ? `₩${Math.round(drRev.gross_revenue).toLocaleString()}` : '—'}</span></div>
                <div className="flex justify-between"><span className="text-[#98A2B3]">플랫폼 수수료 (15%)</span><span className="text-[#98A2B3]">{drRev ? `-₩${Math.round(drRev.peakbridge_fee).toLocaleString()}` : '—'}</span></div>
                <div className="flex justify-between"><span className="text-[#98A2B3]">월 환산</span><span className="text-[#2EBD85] font-bold">{drRev ? `₩${Math.round(drRev.monthly_projection).toLocaleString()}` : '—'}</span></div>
              </div>
            </div>
            <div className="pt-3">
              <p className="text-xs font-medium uppercase tracking-wider text-[#98A2B3] mb-2">차익거래 (경부하 충전 → 최대부하 방전)</p>
              <div className="space-y-1.5 text-sm">
                <div className="flex justify-between"><span className="text-[#98A2B3]">단가 차이</span><span className="text-[#E8ECF1] font-semibold">{arb ? `${arb.charge_price ?? 42.5} → ${arb.discharge_price ?? 147}원/kWh` : '—'}</span></div>
                <div className="flex justify-between"><span className="text-[#98A2B3]">1일 차익</span><span className="text-[#E8ECF1] font-semibold">{arb ? `₩${Math.round(arb.arbitrage).toLocaleString()}` : '—'}</span></div>
                <div className="flex justify-between"><span className="text-[#98A2B3]">ROI</span><span className="text-[#E8ECF1] font-semibold">{arb ? `${arb.roi_percent}%` : '—'}</span></div>
                <div className="flex justify-between"><span className="text-[#98A2B3]">월 환산</span><span className="text-[#2EBD85] font-bold">{arb ? `₩${Math.round(arb.monthly_projection).toLocaleString()}` : '—'}</span></div>
              </div>
            </div>
          </div>
        </Card>
      ...
      <Card title="수요반응 이벤트 이력" subtitle="최근 발령 기록">
        {events.length === 0 ? (
          <p className="text-sm text-[#98A2B3]">아직 발령된 이벤트가 없습니다. 위 버튼으로 발령해보세요.</p>
        ) : (
          <div className="space-y-2">
            {events.map((ev) => (
              <div key={ev.event_id} className="flex items-center justify-between rounded-md border border-[#222933] bg-[#0A0C10] px-4 py-2.5 text-sm">
                <div className="flex items-center gap-3 min-w-0">
                  <span className={`rounded-full px-2 py-0.5 text-xs font-semibold shrink-0 ${
                    ev.action === 'discharge' ? 'bg-[#E8A33D]/15 text-[#E8A33D]'
                    : ev.action === 'charge' ? 'bg-[#4C8DFF]/15 text-[#4C8DFF]'
                    : 'bg-[#222933]/60 text-[#98A2B3]'
                  }`}>
                    {ev.signal_type} {ev.value}
                  </span>
                  <span className="text-[#E8ECF1] truncate">{ev.label}</span>
                </div>
                <span className="font-mono text-xs text-[#98A2B3] shrink-0 ml-3">
