...
### 13차 후반 — 대시보드·리포트 정합성 일괄 정리 (같은 날)
- 24h 전력 프로필: ESS-01 SOC(%)가 전류축에 섞이던 것 수정(unit A만 병합),
  y축 0~0.2A 고정. 루트(/) → /app/ 리다이렉트. CO2·절감액 표기 포맷터
  (formatCo2Kg/formatWon — 실측 스케일 g/소수원 정직 표기).
- 일별 절감액 차트: 예시 폴백 판정을 '절감액>0인 날 5일'로 수정(0행 30개에
  실데이터 모드로 빠지던 것), 커스텀 툴팁(기본 툴팁 라벨 검정색 안 보임 +
  formatter name 인자 오분기 수정), 예시 프로파일도 실측 자릿수로 재생성.
- **전원 절체 로그 신설**(`PowerSourceLog`): 실시간 그래프 위 스트립 —
  현재 한전/ESS방출 배지 + 최근 절체 이벤트(알림 DB 원본 타입 기반, 5초 갱신).
- **ROI 카드 보수화**: 근거 없는 '2.3년' 제거 → 변압기 교체(3.0억) vs ESS(1.8억
  상한+유지) 비교로 코드 계산, 보수 가정(요금절감·보조금·VPP 미반영) 카드에 명시.
